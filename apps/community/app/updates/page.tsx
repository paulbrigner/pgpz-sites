...
const INITIAL_WEEKLY_MEMO_COUNT = 3;

type ArchiveUpdate = Awaited<
  ReturnType<typeof getPublishedPolicyUpdates>
>[number];

function ArchiveUpdateLink({ update }: { update: ArchiveUpdate }) {
  return (
    <Link
      href={update.portalPath}
      className="group block rounded-xl border bg-white/85 p-4 transition hover:border-[rgba(71,85,105,0.55)] hover:shadow-[0_18px_34px_-28px_rgba(30,30,30,0.4)]"
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <ArrowRight
          className="mt-1 h-4 w-4 shrink-0 text-slate-400 group-hover:text-[var(--brand-denim)]"
          aria-hidden="true"
        />
      </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-sm leading-6 text-slate-600">
              Weekly policy memos and special updates are available to active
              PGPZ Community members. Complete membership verification from the
              home page to unlock the archive.
            </p>
            <Button asChild>
...
export default async function UpdatesPage() {
  ...
  const weekly = publishedUpdates.filter(
    (update) => update.category === "weekly",
  );
  const special = publishedUpdates.filter(
    (update) => update.category === "special",
  );
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-5 pb-14">
        ...
        <div className="community-hero__frame">
          <div className="max-w-3xl space-y-5">
            <p className="section-eyebrow text-white/70">
              Member policy archive
            </p>
            <h1 className="text-4xl font-semibold leading-tight sm:text-5xl">
              Weekly memos and special updates for PGPZ members.
            </h1>
            <p className="max-w-2xl text-base leading-7 text-white/78">
              View the latest policy briefing, browse prior weekly updates, and
              revisit special reports that frame major developments for the
              Zcash ecosystem.
            </p>
          </div>
        </div>
      ...
      {latest ? (
        <section className="glass-surface grid gap-5 p-6 lg:grid-cols-[0.42fr_1fr]">
          <Link
            ...
            className="relative block min-h-[18rem] overflow-hidden rounded-2xl border border-[rgba(71,85,105,0.28)] bg-white"
          >
            <Image
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        {[
          {
            eyebrow: "Recurring",
            heading: "Weekly Policy Memos",
            updates: weekly,
            initialCount: INITIAL_WEEKLY_MEMO_COUNT,
          },
          { eyebrow: "Featured", heading: "Special Updates", updates: special },
        ].map(({ eyebrow, heading, updates, initialCount }) => {
          const visibleUpdates = initialCount
            ? updates.slice(0, initialCount)
            : updates;
          const olderUpdates = initialCount ? updates.slice(initialCount) : [];
            ...
            <div key={heading} className="muted-card p-5">
              ...
              <div className="space-y-3">
                {visibleUpdates.map((update) => (
                  <ArchiveUpdateLink key={update.slug} update={update} />
                ))}
                {olderUpdates.length > 0 ? (
                  <details className="group/more">
                    <summary className="flex cursor-pointer list-none items-center justify-center gap-2 rounded-xl border border-[rgba(71,85,105,0.3)] bg-white/70 px-4 py-3 text-sm font-semibold text-[var(--brand-ink)] transition hover:border-[rgba(71,85,105,0.55)] hover:bg-white [&::-webkit-details-marker]:hidden">
                      <span className="group-open/more:hidden">
                        Show {olderUpdates.length} more weekly memo
                        {olderUpdates.length === 1 ? "" : "s"}
                      </span>
                      <span className="hidden group-open/more:inline">
                        Hide older weekly memos
                      </span>
                    </summary>
