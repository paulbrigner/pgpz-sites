...
describe("DocumentLibrary", () => {
  ...
  it("filters adoption independently of archive status and keeps newer uploads separate", () => {
    ...
    expect(screen.getByRole("link", { name: "Download approved copy with signatures" })).toHaveAttribute("href", "/packet");
    expect(screen.getAllByText(/October 1, 2026/)[0]).toBeVisible();
    fireEvent.change(screen.getByRole("combobox", { name: "Filter by document status" }), { target: { value: "all" } });
    expect(screen.getByText("2 documents shown")).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Clear filters" }));
  ...
  it("makes the most recently approved version primary, keeping newer drafts and earlier approvals in collapsed history", () => {
    const approval = { meetingId: "m", resolutionId: "b", resolutionTitle: "Adopt policy", versionId: "approved-v1", sequence: 1, sha256: "hash", adoptedAt: "2026-09-11T12:00:00Z", effectiveTerms: "Effective October 1", signatureCount: 5, directorCount: 5, recordHref: "/record", packetHref: "/approved-packet", originalHref: "/approved-original" };
    const doc = { ...categories[0].documents[0], currentVersionId: "new-draft", adoptions: [{ ...approval, resolutionId: "earlier", versionId: "older-v2", sequence: 2, adoptedAt: "2026-09-10T12:00:00Z", packetHref: "/earlier-packet" }, approval], inEffect: { versionId: "older-v2", sequence: 2, reason: "Currently operative", recordedAt: "2026-09-10", downloadHref: "/operative" } };
    const data = [{ ...categories[0], documents: [doc, { ...doc, documentId: "archived", title: "Archived policy", status: "archived" as const }, { ...doc, documentId: "unapproved", title: "Proposal", adoptions: [] }] }];
    render(<DocumentLibrary categories={data} />);
    const approvedView = screen.getByRole("button", { name: "Board-approved copies (1)" });
    fireEvent.click(approvedView);
    expect(approvedView).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("combobox", { name: "Filter by adoption" })).toHaveValue("adopted");
    expect(screen.getByText("1 document shown")).toBeVisible();
    expect(screen.getByRole("link", { name: "Conflict of Interest Policy" })).toHaveAttribute("href", "/approved-packet");
    expect(screen.getByRole("link", { name: "Download approved copy with signatures" })).toHaveAttribute("href", "/approved-packet");
    expect(screen.getByText("Board approved · v1")).toBeVisible();
    expect(screen.getByText("In effect · v2")).toBeVisible();
    expect(screen.getByText(/Adopted Sep 11, 2026, 8:00 AM Eastern/)).toBeVisible();
    expect(screen.queryByText("Version history")).not.toBeInTheDocument();
    expect(screen.getByText("Adoption records · earlier version adopted").closest("details")).not.toHaveAttribute("open");
    ...
    fireEvent.click(screen.getByRole("button", { name: "All library documents" }));
    expect(screen.getByRole("combobox", { name: "Filter by adoption" })).toHaveValue("all");
    fireEvent.click(screen.getByRole("button", { name: /Policies/ }));
    expect(screen.getByRole("link", { name: "Proposal" })).toHaveAttribute("href", doc.downloadHref);
    expect(screen.queryByRole("link", { name: "Archived policy" })).not.toBeInTheDocument();
  });
