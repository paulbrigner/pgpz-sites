...
import { DISCLOSURE_CATEGORIES, DISCLOSURE_ELECTRONIC_CONSENT, disclosureAcknowledgment, disclosureDirector, disclosureStatus, disclosureOutcome, disclosureReviewEmailStatus, type DisclosureCandidate, type DisclosureForm, type DisclosureView } from "@/lib/disclosures";
...
export function DisclosureWorkspace({ initial, candidates }: { initial: DisclosureView; candidates: DisclosureCandidate[] }) {
  ...
  async function review(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const detail = `${isReviewer ? disclosureOutcome(String(data.get("outcome"))) : "Counsel advice recorded"}. Your note for signed revision ${request.revision} has been saved. You do not need to submit it again.${isReviewer ? ` ${disclosureReviewEmailStatus(result.request?.reviewNotice)}` : ""}`;
  ...
  return <div className="mt-7 space-y-6">
    ...
    {(isReviewer || isCounsel) && request.revision > 0 && <Surface className="p-6"><h2 className="text-xl font-semibold">{isReviewer ? showReviewReceipt ? "Your recorded review" : "Record your review" : "Add counsel’s advice"}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Review signed revision {request.revision}. Record relevant findings, required disclosures to other decision-makers, recusals, and follow-up actions. This step does not approve compensation, a conflicted transaction, or a Board resolution.</p><p className="mt-2 text-sm leading-6 text-[var(--muted)]">The person disclosing can read these notes. Keep directors-only deliberation in a restricted executive session.</p>{isReviewer && !showReviewReceipt && <p className="mt-2 text-sm leading-6 text-[var(--muted)]">Recording an outcome automatically emails the person disclosing a status and portal link. Private review notes stay in the portal.</p>}{(reviewFeedback || showReviewReceipt) && <div ref={reviewNotice} tabIndex={-1} role={reviewFeedback?.kind === "error" ? "alert" : "status"} className={`mt-5 rounded-xl border p-5 focus:outline-2 focus:outline-offset-2 ${reviewFeedback?.kind === "error" ? "border-red-300 bg-red-50 text-red-900" : "border-emerald-300 bg-emerald-50 text-emerald-950"}`}><h3 className="text-lg font-semibold">{reviewFeedback?.kind === "error" ? "Review not confirmed" : isReviewer ? "Review recorded" : "Advice recorded"}</h3><p className="mt-2 text-sm leading-6">{reviewFeedback?.detail ?? `${disclosureStatus(request.status)}. Your review of signed revision ${request.revision} is saved in the review history below. ${disclosureReviewEmailStatus(request.reviewNotice)}`}</p></div>}
