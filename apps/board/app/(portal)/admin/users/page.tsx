...
import { canManageBoardUsers, requireBoardAdministration } from "@/lib/session";
...
export default async function BoardUsersPage() {
  const administrator = await requireBoardAdministration("/admin/users");
  if (!canManageBoardUsers(administrator)) notFound();
  ...
  return <Container className="py-10 sm:py-14"><section className="max-w-3xl"><Badge tone="accent">Privileged access</Badge><h1 className="mt-5 text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">User management</h1><p className="mt-3 text-base leading-7 text-[var(--muted)]">Manage the people authorized to use the Board portal, their roles, sessions, and passwordless enrollment.</p></section><BoardUserManager initialUsers={users} currentUserEmail={administrator.email} /></Container>;
}
