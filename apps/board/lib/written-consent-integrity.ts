...
import { resolutionReviewRecordHash } from "@/lib/resolution-review-integrity";

export function consentPayload(ballot: Pick<BoardAsyncBallot, "id" | "meetingId" | "title" | "motion" | "attachments" | "eligibleVoters" | "adoption" | "review">, consent: Pick<WrittenConsent, "rosterRevision" | "startAt" | "endAt" | "statement" | "withdrawalStatement"> & { schema?: 1 | 2 | 3 | 4 }) {
  ...
  return { schema, meetingId: ballot.meetingId, ballotId: ballot.id, title: ballot.title, motion: ballot.motion,
    ...
    ...(schema === 4 ? { reviewRecord: { roundId: ballot.review?.round?.id, sha256: resolutionReviewRecordHash(ballot.review) } } : {}),
  };
}
