...
export function hasVerifiedAdoption(ballot: BoardAsyncBallot): boolean {
  ...
  if (!consent || consent.schema < 2 || !ballot.adoption || ballot.status !== "closed" || ballot.result?.outcome !== "passed" || !ballot.closedAt || !ballot.eligibleVoters.length) return false;
