...
export function consentRecord(ballot: BoardAsyncBallot, receipts: readonly ConsentReceipt[], viewerEmail: string) {
  ...
  return {
    ...
    attachments: (ballot.attachments || []).map(({ description, ...doc }) => ({ ...doc, ...((ballot.consent?.schema || 1) >= 3 && description ? { description } : {}) })), directors: ballot.eligibleVoters,
    adoption: (ballot.consent.schema >= 2) ? ballot.adoption || null : null,
    canonicalPayload: consentPayload(ballot, ballot.consent),
    reviewRecord: ballot.consent.schema === 4 ? consentPayload(ballot, ballot.consent).reviewRecord : null,
...
export function consentRecordHtml(record: ReturnType<typeof consentRecord>) {
  ...
  ${record.reviewRecord ? `<h2>Associated review record</h2><p>The finalized director review record is bound to this consent by its digest. Detailed reviews remain restricted to active directors.</p><p class="meta">Round: ${escape(record.reviewRecord.roundId)}<br>Review record SHA-256: ${escape(record.reviewRecord.sha256)}</p>` : ""}
