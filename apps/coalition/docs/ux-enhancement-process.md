...
This document explains how designers should use the PGPZ Figma workspace and Codex together to improve the PGPZ Coalition website without losing track of the current product behavior, review decisions, or implementation requirements.
...
Use the shared Figma file named:

`pgpz-coalition UX Refinement Workspace`

Current workspace URL:

`https://www.figma.com/design/ZvgqGikabHrG1bNY8ZsY3K`

This repo mirrors the structure of the `pgpz-community UX Refinement Workspace`, with Coalition-specific captures, routes, palette notes, member profile fields, and membership states. The Figma workspace contains editable documentation boards plus imported local-page captures.

Important pages:

- `00 Cover + resource index`: landing page for the resource set and palette notes.
- `01 Current flow map`: system flow anchors named `F01`, `F02`, etc.
- `07 Screen inventory + flow links`: current screen inventory named `S01`, `S02`, etc.
- `08 Screen captures`: baseline screenshots of the current Coalition website, cloned from the imported capture page.
- `02 UX audit backlog`: intake board for candidate UX improvements.
- `09 UX enhancement process`: visual process map.
- `10 Redesign work packet template`: duplicable packet template for each screen or flow slice.
- `05 Decisions + handoff`: approval, Codex handoff, QA, and traceability board.
- `99 Scratch - imported captures`: raw imported captures from the running app; do not use as the primary documentation surface.

## Terms

- **Screen ID (`Sxx`)**: a current product screen from the screen inventory, such as `S06 Admin invite member`.
- **Flow anchor (`Fxx`)**: a step in the system flow map, such as `F07 Activate invitation`.
...
- `S06 invitation activation recovery states`
- `S02 signup profile confidence and legal clarity`
...
Open `10 Redesign work packet template`.

Duplicate the top-level `Redesign work packet template` frame and move the duplicate to the design exploration page or another clearly named area. Rename it using this format:
...
```text
S06 - Invitation activation - redesign packet - 2026-06-19
S02 - Signup form clarity - redesign packet - 2026-06-19
```
...
5. Rename it `S06 - Invitation activation - redesign packet - 2026-06-19`.
6. Fill in the problem: invited members need clear recovery when an activation link is expired, already used, or invalid.
7. Fill in states: default activation success, expired token, invalid token, already active, signed-out follow-up, and mobile.
