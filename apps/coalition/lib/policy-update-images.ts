...
export function isPolicyUpdateDisplayImageAllowed(image: PolicyUpdateImageLike) {
  ...
  if (/\/(?:member-join-qr|community-join-qr|coalition-join-qr)\.(?:png|jpe?g|webp)$/i.test(src)) {
    return false;
  }
  ...
  if (/\bqr code for joining the pgpz (?:community|coalition)\b/i.test(text) && !/\bsignal\b/i.test(text)) {
    return false;
  }

  if (/\bpgpz (?:community|coalition) signup qr\b/i.test(text)) {
    return false;
  }
