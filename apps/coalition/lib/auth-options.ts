...
async function assertLegalAcceptanceForAccountEmail(identifier: string, url: string) {
  ...
  if (!signupProfileId) {
    throw new Error(
      "Create an account from the sign-up page and accept the Terms of Service, Privacy Policy, and Coalition Guidelines before requesting an email link."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Please accept the current Terms of Service, Privacy Policy, and Coalition Guidelines before creating an account."
    );
  }
}
