...
function stripPdfChrome(value: string) {
  return value
    .replace(/PGPZ Coalition\s+Member Policy Resource/gi, "")
    .replace(/(?:community|coalition)\.pgpz\.org\s*\|\s*PGPZ Coalition\s*\|\s*Page\s+\d+/gi, "")
    .replace(/\n?\s*--- Page \d+ of \d+ ---\s*\n?/gi, "\n")
    .replace(/https:\/\/(?:community|coalition)\.pgpz\.org\/updates\/[^\s]+/gi, "")
    .replace(/\bNot a PGPZ member\?\s*Sign up here:?\b/gi, "")
    .replace(
      /We are excited to announce the launch of the PGPZ Coalition Signal chat[\s\S]*?scan the QR code to get started!?/gi,
      "",
    );
}
...
function splitSourceLines(value: string, { keepBlank = false }: { keepBlank?: boolean } = {}) {
  const lines = sourceTextForExactConversion(value)
    ...
    .filter((line) => !/^PGPZ Coalition$/i.test(line))
...
function imageMetadataFromLink({
  ...
  if (lowerHref.includes("coalition.pgpz.org") && !lowerHref.includes("signal.group")) {
    return null;
  }
  ...
  if (lowerHref.includes("signal.group")) {
    return {
      ...
      alt: "PGPZ Coalition Signal chat QR code",
      caption: "Scan to join the PGPZ Coalition Signal chat.",
      href,
    };
  }
...
function mergeExtractedImagesIntoContent(
  ...
  for (const image of images) {
    ...
    if (image.role === "signal-chat") {
      if (!appendImageToFirstMatchingSection(sections, image, /\bsignal\b|\b(?:community|coalition) chat\b/i)) {
        sections.unshift({ heading: "PGPZ Coalition Signal Chat", body: [], images: [image] });
      }
      continue;
    }
