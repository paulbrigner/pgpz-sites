...
function stripPdfChrome(value: string) {
  return value
    .replace(/PGPZ (?:Community|Coalition)\s+Member Policy Resource/gi, "")
    .replace(/(?:community|coalition)\.pgpz\.org\s*\|\s*PGPZ (?:Community|Coalition)\s*\|\s*Page\s+\d+/gi, "")
    .replace(/\n?\s*--- Page \d+ of \d+ ---\s*\n?/gi, "\n")
...
function imageMetadataFromLink({
  ...
  if (/(?:community|coalition)\.pgpz\.org/.test(lowerHref) && !lowerHref.includes("signal.group")) {
    return null;
  }
