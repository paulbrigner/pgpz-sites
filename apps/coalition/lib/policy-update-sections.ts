...
function isPortalIntroSection(section: PolicyUpdateSection) {
  const text = `${section.heading} ${section.body.join(" ")}`.toLowerCase();
  return /\bsignal chat\b|\bpgpz (?:community|coalition) signal\b|\bjoin the pgpz (?:community|coalition)\b/.test(text);
}
