...
export const normalizeBaseUrl = (baseUrl?: string | null) =>
  (baseUrl || SITE_URL || "https://coalition.pgpz.org").replace(/\/+$/, "");
...
export function renderMemberEmailFooter({
  ...
  return `<p style="margin:0;color:${brandedEmailColors.slate};font-size:13px;line-height:1.6;">You are receiving this because your PGPZ Coalition membership is active. Visit <a href="${escapeHtml(url)}" style="color:${brandedEmailColors.goldDeep};">PGPZ Coalition</a> for member resources.${optOut}</p>`;
}

export function renderForwardedEmailCoalitionCta({
  portalUrl,
  href,
}: {
  portalUrl?: string | null;
  ...
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0 0;border:1px solid ${colors.line};border-radius:16px;background:#FFFDF5;overflow:hidden;">
    <tr>
      <td style="padding:18px 18px 18px 20px;vertical-align:middle;">
        ...
        <p style="margin:0 0 12px;color:${colors.slate};font-size:14px;line-height:1.62;">Request PGPZ Coalition access to receive updates directly and coordinate with approved members.</p>
...
export function renderForwardedEmailCoalitionText(portalUrl?: string | null) {
  const url = normalizeBaseUrl(portalUrl);
  return [
    "Did someone forward you this email?",
    "Request PGPZ Coalition access to receive updates directly and coordinate with approved members.",
    url,
  ].join("\n");
}
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:760px;background:#ffffff;border:1px solid ${colors.line};border-radius:18px;overflow:hidden;box-shadow:0 22px 48px rgba(30,30,30,0.12);">
            <tr>
              <td style="background:linear-gradient(135deg,${colors.coal},#143A3D 60%,${colors.denim});padding:28px 30px;color:#ffffff;">
