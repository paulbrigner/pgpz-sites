...
describe("buildPolicyUpdateEmail", () => {
  ...
  it("renders policy update section images through unauthenticated email asset URLs", () => {
    ...
    expect(built.html).toContain(
      'src="https://coalition.pgpz.org/api/policy-updates/test-upload/email-assets/x-josh-swihart.png?v=materialization-1&amp;sig=',
    );
  ...
  it("adds tracked links, an open pixel, and unsubscribe URL when tracking is enabled", () => {
    ...
    expect(built.text).toContain("Unsubscribe from policy updates: https://coalition.pgpz.org/api/email/unsubscribe/policy-track-123");
    expect(built.unsubscribeUrl).toBe(
      "https://coalition.pgpz.org/api/email/unsubscribe/policy-track-123",
    );
  });
});
