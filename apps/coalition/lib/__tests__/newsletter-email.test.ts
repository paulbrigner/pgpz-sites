...
describe("buildNewsletterEmail", () => {
  ...
  it("adds tracking links, an open pixel, and unsubscribe URL when tracking is enabled", () => {
    ...
    expect(built.text).toContain("Unsubscribe from newsletters: https://coalition.pgpz.org/api/email/unsubscribe/track-123");
    expect(built.unsubscribeUrl).toBe("https://coalition.pgpz.org/api/email/unsubscribe/track-123");
  });
});
