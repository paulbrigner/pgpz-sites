...
import {
  ...
  escapeHtml,
  normalizeBaseUrl,
  renderForwardedEmailCoalitionCta,
  renderForwardedEmailCoalitionText,
...
function policyUpdateEmailIntro(update: Pick<PolicyUpdate, "category" | "summary">) {
  ...
  if (!summary) {
    return update.category === "weekly"
      ? "This week's PGPZ Coalition policy memo is now available."
      : "This PGPZ Coalition update is now available.";
  }
  ...
  const prefix =
    update.category === "weekly"
      ? "This week's PGPZ Coalition policy memo covers"
      : "This PGPZ Coalition update covers";
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:760px;background:#ffffff;border:1px solid ${colors.line};border-radius:18px;overflow:hidden;box-shadow:0 22px 48px rgba(30,30,30,0.12);">
            <tr>
              <td style="background:linear-gradient(135deg,${colors.coal},#143A3D 60%,${colors.denim});padding:28px 30px;color:#ffffff;">
                <div style="font-size:12px;letter-spacing:0.26em;text-transform:uppercase;font-weight:700;color:${colors.goldSoft};">PGPZ Coalition</div>
