...
export async function requestManualApproval(userId: string) {
  ...
  try {
    await documentClient.update({
      ...
      ConditionExpression:
        `attribute_exists(#pk) AND (attribute_not_exists(#membershipStatus) OR #membershipStatus = :none) AND (attribute_not_exists(#manualApprovalStatus) OR #manualApprovalStatus <> :pending) AND ${ACTIVE_ACCOUNT_CONDITION}`,
    ...
    if (err?.name === "ConditionalCheckFailedException") {
      ...
      if (
        ...
        latest.Item.membershipStatus !== "invited" &&
