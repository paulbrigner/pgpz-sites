...
function buildWelcomeEmail(user: UserRecord, to: string) {
  ...
  const html = `
    <p>Hi ${name || "there"},</p>
    <p>Welcome to the PGPZ Coalition. Your membership is active and you can sign in any time to access coalition resources, messaging, and campaign materials.</p>
    <p>If you have questions, reply to this email and we will help.</p>
    <p>Thanks,<br/>PGPZ Coalition Team</p>
  `;
  ...
  try {
    ...
    if (type === "welcome") {
      ...
      if (customSubject || customHtml || customText) {
        subject = customSubject || "Welcome to PGPZ Coalition";
