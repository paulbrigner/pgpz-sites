...
describe("public file delivery", () => {
  ...
  it("returns 404 without reading metadata when the registered feature is off", async () => {
    mocks.featureEnabled.mockReturnValue(false);
    const { GET } = await import("./route");
    const response = await GET(
      new Request("https://site.example/resources/statements/test.pdf") as any,
      context,
    );
  ...
  it("streams a byte range for browser PDF viewing", async () => {
    ...
    const { GET } = await import("./route");
    const response = await GET(
      new Request("https://site.example/resources/statements/test.pdf", {
