...
export default function AdminClient({ initialRoster, currentAdminId }: Props) {
  ...
  const reactivateMember = async (member: AdminMember) => {
    ...
    try {
      ...
      setNotice(`User reactivated for ${member.email || displayName(member)}. Membership remains unapproved.`);
      await loadRoster();
    } catch (err: any) {
                          ...
                          <div className="rounded-md border border-[rgba(31,76,111,0.22)] bg-white p-3">
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                member.isAdmin
                                  ? "border-[rgba(47,111,104,0.55)] bg-[var(--zcash-gold-soft)]"
                          ...
                          <div className="rounded-md border border-rose-200 bg-white p-3">
                            ...
                            {deactivated ? (
                              <p className="mt-2 text-xs text-slate-600">
                                Reactivation restores sign-in only. Membership must be approved or accepted again.
                              </p>
                            ) : null}
                          </div>
                        </div>
