...
export type EmailLogRuntimeDependencies = {
  documentClient: EmailRuntimeDocumentClient;
  tableName: string;
};

export function createEmailLogRuntime({
  documentClient,
  tableName: TABLE_NAME,
}: EmailLogRuntimeDependencies) {
...
function groupPolicyUpdateEmailLogs(
...
async function recordEmailEvent(params: EmailLogParams) {
...
async function summarizePolicyUpdateEmailStats(slugs: string[]) {
...
async function recordPolicyUpdateSendRun({
  sendRunId,
  update,
  recipientCount,
  sentCount,
...
async function updatePolicyUpdateSendRunProgress({
  sendRunId,
  sentCount,
  failedCount,
  failurePreview,
...
async function listPolicyUpdateSendHistory(updates: PolicyUpdateHistoryContext[]) {
