...
import type { EmailTransportFactory } from "./types";

type AsyncDependency = (...args: any[]) => any;
type PolicyUpdate = any;
type AdminSignupNotificationJobPayload = any;

export type EmailBackgroundJobProcessorDependencies = {
  assertSmokeRecipient: AsyncDependency;
  completeBackgroundJobTask: AsyncDependency;
  getCurrentEligibleRecipient: AsyncDependency;
  listBackgroundJobs: AsyncDependency;
  listBackgroundJobTasks: AsyncDependency;
  markBackgroundJobDeliveryStarted: AsyncDependency;
  markBackgroundJobTaskProjectionCompleted: AsyncDependency;
  releaseBackgroundJobTaskForRetry: AsyncDependency;
  buildEmailServerConfig: AsyncDependency;
  bindNewsletterTrackingDestinations: AsyncDependency;
  createNewsletterTrackingRecord: AsyncDependency;
  markNewsletterTrackingSent: AsyncDependency;
  recordEmailEvent: AsyncDependency;
  updatePolicyUpdateSendRunProgress: AsyncDependency;
  claimNewsletterBackgroundDelivery: AsyncDependency;
  markNewsletterSent: AsyncDependency;
  updateNewsletterSendRunProgress: AsyncDependency;
  buildAdminSignupNotificationEmail: AsyncDependency;
  getCurrentEligibleAdminSignupNotificationRecipient: AsyncDependency;
  emailFrom: string | null | undefined;
  siteUrl: string;
  listUnsubscribeHeaders: AsyncDependency;
  buildNewsletterEmail: AsyncDependency;
  buildPolicyUpdateEmail: AsyncDependency;
  createTransport: EmailTransportFactory;
};

export function createEmailBackgroundJobProcessor({
  ...
  createTransport,
}: EmailBackgroundJobProcessorDependencies) {
...
async function failurePreview(jobId: string) {
  ...
  return tasks
    .filter((task: BackgroundJobTaskRecord) => task.status === "failed" || task.status === "delivery_unknown")
    .map((task: BackgroundJobTaskRecord) => ({
...
async function processEmailBackgroundJobTask({
  job,
  task,
  leaseToken,
}: {
...
async function reconcileEmailBackgroundJobProjections(limit = 100) {
  ...
  for (const job of jobs) {
    ...
    for (const task of tasks.filter(
      (candidate: BackgroundJobTaskRecord) =>
