...
import { normalizeRecipients as normalizeBackgroundJobRecipients } from "./recipients";
import { sanitizeJobError } from "./errors";
import { backgroundJobExpiration } from "./retention";
import type {
  BackgroundJobKind,
...
type DocumentClientAdapter = {
  ...
  batchWrite(input: Record<string, unknown>): Promise<any>;
  transactWrite(input: Record<string, unknown>): Promise<any>;
};

export type BackgroundJobQueueBatch = {
  QueueUrl: string;
  Entries: Array<{
    Id: string;
    MessageBody: string;
...
export type BackgroundJobRuntimeDependencies = {
  documentClient: DocumentClientAdapter;
  applicationTableName: string;
  normalizeEmail(value: unknown): string;
  sendMessageBatch(input: BackgroundJobQueueBatch): Promise<{
    Successful?: Array<{ Id?: string }>;
    Failed?: Array<{ Id?: string; Message?: string }>;
  }>;
};

let runtimeDependencies: BackgroundJobRuntimeDependencies | null = null;

export function configureBackgroundJobRuntime(
...
function runtime() {
  if (!runtimeDependencies) {
    throw new Error("Background-job runtime dependencies have not been configured.");
  }
  return runtimeDependencies;
}

const documentClient: DocumentClientAdapter = {
  get: (input) => runtime().documentClient.get(input),
  put: (input) => runtime().documentClient.put(input),
  query: (input) => runtime().documentClient.query(input),
  update: (input) => runtime().documentClient.update(input),
  batchWrite: (input) => runtime().documentClient.batchWrite(input),
  transactWrite: (input) => runtime().documentClient.transactWrite(input),
};

const normalizeEmail = (value: unknown) => runtime().normalizeEmail(value);
const applicationTableName = () => runtime().applicationTableName;
...
export async function listBackgroundJobTasksPage(
  ...
  return {
    tasks: (response.Items || [])
      .map((item: Record<string, any>) => toTask(item))
...
async function dispatchTasks(jobId: string, tasks: BackgroundJobTaskRecord[]) {
  ...
  for (let offset = 0; offset < tasks.length; offset += 10) {
    ...
    const response = await runtime().sendMessageBatch({
      QueueUrl: queueUrl,
...
export async function listBackgroundJobsPage({
  ...
  return {
    jobs: (response.Items || [])
      .map((item: Record<string, any>) => toJob(item))
