...
export type AccessLogDocumentClient = {
...
export type AccessLogRuntimeDependencies = {
  documentClient: AccessLogDocumentClient;
  tableName: string;
};

let configuredRuntime: AccessLogRuntimeDependencies | null = null;

export function configureAccessLogRuntime(dependencies: AccessLogRuntimeDependencies) {
  configuredRuntime = dependencies;
}

const runtime = () => {
  if (!configuredRuntime) throw new Error("Access-log runtime is not configured.");
...
export async function listAccessLog(options: ListAccessLogOptions = {}) {
  ...
  do {
    ...
    const pageEvents = (res.Items || [])
      .map((item: Record<string, any>) => toAccessLogEvent(item))
      .filter((event: AccessLogEvent | null): event is AccessLogEvent => !!event);
