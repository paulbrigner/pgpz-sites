    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 animate-pulse">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="space-y-2 rounded-2xl border border-slate-200/60 bg-white/80 p-4 shadow-sm">
      ...
      <div className="space-y-3 rounded-2xl border border-slate-200/70 bg-white/80 p-5 shadow-sm">
        ...
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="flex items-center gap-3 rounded-xl border border-slate-200/70 bg-white/70 p-3">
