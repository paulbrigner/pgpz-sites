...
import type {
  NewsletterMailerButton,
  NewsletterMailerConfig,
} from "./contracts";
import {
...
export type NewsletterApiState = {
...
export function NewsletterMailer({
  ...
  Button: NewsletterMailerButton;
  config?: Partial<NewsletterMailerConfig>;
}) {
  const config = { ...DEFAULT_NEWSLETTER_MAILER_CONFIG, ...configOverrides };
  ...
  const loadState = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(config.newslettersEndpoint, { cache: "no-store" });
  ...
  const updateForm = (field: keyof typeof emptyNewsletterForm, value: string) => {
  ...
  const saveDraft = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch(config.newslettersEndpoint, {
        method: "POST",
  ...
  const sendNewsletter = async () => {
    ...
    try {
      ...
      if (res.status === 202 && body.queued && body.job) {
        ...
        setBackgroundJob({
          job: body.job,
          statusUrl: body.statusUrl || `${config.jobsEndpoint}?jobId=${encodeURIComponent(body.job.id)}`,
          active: true,
        });
        setConfirmSend(false);
        setNotice(
  ...
  const deleteDraft = async (newsletter: Newsletter) => {
    ...
    try {
      ...
      if (form.id === newsletter.id) setForm(emptyNewsletterForm);
        ...
        <div className="rounded-2xl border bg-white/90 p-5">
          ...
          <div className="mt-5 space-y-4">
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[rgba(var(--program-accent-rgb),0.32)] bg-[var(--brand-ice)] p-4 text-sm leading-6 text-slate-700">
              ...
              <span>
                {draftSend
                  ? `I understand this will save this newsletter draft and send it to ${selectedAudienceLabel} without tracking or send history.`
